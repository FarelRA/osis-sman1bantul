    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8 rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-800 aspect-video">
                <img src="<?= asset('assets/images/' . $club['logo']) ?>" class="w-full h-full object-cover"
            ...
            <div class="text-center mb-12">
                ...
                <?php if (!empty($club['category']) || !empty($club['members'])): ?>
                    <div class="flex items-center justify-center gap-4 mt-4">
                        <?php if (!empty($club['category'])): ?>
                            ...
                            $catColor = $catColors[$club['category']] ?? 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300';
                            ?>
                            <span class="badge <?= $catColor ?>"><?= htmlspecialchars($club['category']) ?></span>
                        <?php endif; ?>
                        <?php if (!empty($club['members'])): ?>
                            <span class="text-sm text-gray-500 dark:text-gray-400"><?= (int) $club['members'] ?>+ members</span>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="card p-8">
                ...
                <?php if ($formUrl = getFormUrl('club', $club['slug'])): ?>
                    ...
                    <?php
                    $socials = [];
                    if (!empty($club['instagram'])) $socials[] = ['name' => 'Instagram', 'url' => $club['instagram'], 'color' => 'hover:text-pink-600'];
                    if (!empty($club['youtube'])) $socials[] = ['name' => 'YouTube', 'url' => $club['youtube'], 'color' => 'hover:text-red-600'];
                    if (!empty($club['tiktok'])) $socials[] = ['name' => 'TikTok', 'url' => $club['tiktok'], 'color' => 'hover:text-gray-900 dark:hover:text-white'];
                    if (!empty($socials) || !empty($club['links'])): ?>
                        <div class="border-t border-gray-100 dark:border-gray-800 pt-6 mt-6">
                            <div class="flex flex-wrap items-center gap-4">
                                ...
                                <?php endforeach; ?>
                                <?php foreach ($club['links'] ?? [] as $link): ?>

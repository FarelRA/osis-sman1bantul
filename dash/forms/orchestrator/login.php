...
if (isset($_SESSION['orchestrator_account']) && isset($_SESSION['orchestrator_form_id'])) {
    if ($_SESSION['orchestrator_form_id'] === $formId) {
        header('Location: ' . ADMIN_PATH . '/forms/orchestrator.php?form_id=' . urlencode($formId));
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!CSRF::verify($csrfToken)) {
    ...
    } elseif (!Security::checkRateLimit($_SERVER['REMOTE_ADDR'] ?? 'unknown')) {
        ...
        if (empty($username) || empty($password)) {
            ...
            Security::incrementRateLimit($_SERVER['REMOTE_ADDR'] ?? 'unknown');
        } else {
            ...
            if ($account) {
                ...
                Security::resetRateLimit($_SERVER['REMOTE_ADDR'] ?? 'unknown');

                header('Location: ' . ADMIN_PATH . '/forms/orchestrator.php?form_id=' . urlencode($formIdPost));
                exit;
            } else {
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8">
            ...
            <div class="mt-6 pt-6 border-t border-gray-100 dark:border-gray-700 text-center">
                <p class="text-sm text-gray-500">
                    Are you an admin?
                    <a href="<?= ADMIN_PATH ?>/forms/login.php" class="text-amber-600 hover:text-amber-700 font-bold">Admin Login</a>
                </p>
            </div>
        </div>

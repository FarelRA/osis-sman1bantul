...
switch ($action) {
    ...
    // ===== Lookup by Nomer Induk (OCR) =====

    case 'lookup_by_nomer_induk':
        if (!$formId) {
            echo json_encode(['success' => false, 'error' => 'Missing form_id']);
            exit;
        }

        $nomerInduk = trim($input['nomer_induk'] ?? '');
        if (!$nomerInduk) {
            echo json_encode(['success' => false, 'error' => 'Missing nomer_induk']);
            exit;
        }
        ...
        foreach ($registrations as $reg) {
            $regNi = trim($reg['data']['nomer_induk'] ?? '');
            if ($regNi === $nomerInduk) {
                $match = $reg;
                break;
            }
        ...
        if (!$match) {
            echo json_encode(['success' => false, 'error' => 'No participant found with Nomer Induk: ' . $nomerInduk]);
            exit;
        }

        $orchestratorStatus = [
            'checked_in' => $orchRepo->hasCheckedIn($formId, $match['id']),
            'merch_collected' => $orchRepo->hasCollectedMerch($formId, $match['id']),
            'current_location' => $orchRepo->getCurrentLocation($formId, $match['id'])
        ];
    ...
    case 'ocr_with_ai':
        ...
        // Load global settings for AI config
        $formsFile = BASE_PATH . '/data/forms.json';
        $formData = file_exists($formsFile) ? json_decode(file_get_contents($formsFile), true) : [];
        $globalSettings = $formData['settings'] ?? [];
        $aiApiKey = $globalSettings['ai_api_key'] ?? '';
        $aiApiUrl = $globalSettings['ai_api_url'] ?? 'https://api.openai.com/v1';
        $aiModel = $globalSettings['ai_model'] ?? 'gpt-4o';

        if (!$aiApiKey) {
            @unlink($tmpPath);
            echo json_encode(['success' => false, 'error' => 'AI API key not configured in Settings']);
            exit;
        }

        require_once __DIR__ . '/../../../src/Core/AIClient.php';

        $extraParams = !empty($globalSettings['ai_extra_params']) ? json_decode($globalSettings['ai_extra_params'], true) : [];
        $client = new AIClient($aiApiKey, $aiApiUrl, $aiModel, $extraParams ?: []);
        $result = $client->extractOcrText($tmpPath);
        @unlink($tmpPath);

        if (!$result['success']) {
            echo json_encode(['success' => false, 'error' => $result['error'] ?? 'AI OCR failed']);
            exit;
        }
    ...
    case 'get_recent_broadcasts':
        ...
        $since = date('c', strtotime('-5 minutes'));
        $result = $orchRepo->getEventsByForm($formId, [
            'type' => 'broadcast',
            'since' => $since
        ]);
        ...
        foreach ($result['events'] as $event) {
            ...
            $broadcasts[] = [
                ...
                'participant_name' => $reg['data']['full_name'] ?? $event['reg_id']
            ];
        }

...
if (!$isFullAdmin && !$isFormsAdmin && !$isOrchestratorUser) {
    ...
    if ($activeFormId) {
        header('Location: ' . ADMIN_PATH . '/forms/orchestrator/login.php?form_id=' . urlencode($activeFormId));
    } else {
...
// Merch toggle — from form registration_settings, default true
$merchEnabled = $activeForm['registration_settings']['merch_enabled'] ?? true;

// OCR mode — 'tesseract' (on-device) or 'ai' (server-side LLM), default tesseract
$ocrMode = $activeForm['registration_settings']['ocr_mode'] ?? 'tesseract';
...
if (!$activeForm) {
    if ($isOrchestratorUser) {
        header('Location: ' . ADMIN_PATH . '/forms/orchestrator/login.php');
    } else {
...
if (!$canScanAndDashboard) {
    header('Location: ' . ADMIN_PATH . '/forms/orchestrator-accounts.php?form_id=' . urlencode($activeFormId));
    exit;
}

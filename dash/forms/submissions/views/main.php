<div class="mb-6 sm:mb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm sm:text-base text-gray-500">Submissions Dashboard</p>
        </div>
...
<div id="statusFilters" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3 md:gap-4 mb-6 sm:mb-8">
    ...
    <div data-filter-status="pending"
        ...
        <div id="stat-pending" class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">-</div>
        <div class="text-[10px] sm:text-xs bg-yellow-500 text-white px-1.5 sm:px-2 py-0.5 rounded-full inline-block mt-1">
            Pending
        </div>
    </div>
    <div data-filter-status="verified"
        class="status-filter bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center shadow border-2 border-transparent cursor-pointer hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600 transition-all">
        <div id="stat-verified" class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">-</div>
        <div class="text-[10px] sm:text-xs bg-green-500 text-white px-1.5 sm:px-2 py-0.5 rounded-full inline-block mt-1">
            Verified
        </div>
    </div>
    <div data-filter-status="failed"
        class="status-filter bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center shadow border-2 border-transparent cursor-pointer hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600 transition-all">
        <div id="stat-failed" class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">-</div>
        <div class="text-[10px] sm:text-xs bg-red-500 text-white px-1.5 sm:px-2 py-0.5 rounded-full inline-block mt-1">
            Failed
        </div>
    </div>
</div>
...
<div id="submissionContainer" class="relative">
    <!-- Loading State -->
    <div id="loadingState" class="flex items-center justify-center py-12">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
        <span class="ml-3 text-gray-500">Loading submissions...</span>
    </div>
    ...
    <div id="emptyState" class="hidden bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl p-8 sm:p-12 text-center border-2 border-dashed border-gray-200 dark:border-gray-700">
        ...
        <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-2">No Submissions Found</h3>
        <p id="emptyMessage" class="text-sm sm:text-base text-gray-500 dark:text-gray-400">Submissions for this form will appear here.</p>
    </div>
    ...
    <div id="virtualViewport" class="virtual-viewport relative">
        <div id="virtualContent" class="relative" style="height: 0;">
            <div id="submissionList" class="relative"></div>
        </div>
    </div>
</div>
...
<link rel="stylesheet" href="<?= ADMIN_PATH ?>/forms/submissions/assets/submissions.css?v=<?= time() ?>">
<script>
    window.SUBMISSIONS_CONFIG = {
        formId: <?= json_encode($activeFormId) ?>,
        csrfToken: <?= json_encode($csrfToken) ?>,
        formSteps: <?= json_encode($activeForm['steps'] ?? []) ?>,
        displayFields: <?= json_encode($displayFields ?? []) ?>,
        fieldLabels: <?= json_encode($fieldLabels ?? []) ?>,
        apiUrl: '<?= ADMIN_PATH ?>/forms/submissions/api.php'
    };
</script>
<script src="<?= ADMIN_PATH ?>/forms/submissions/assets/submissions.js?v=<?= time() ?>"></script>

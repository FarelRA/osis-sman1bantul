...
<head>
    ...
    <link rel="icon" type="image/png" href="<?= asset('assets/images/osis.webp') ?>">
...
<body class="bg-gray-50 dark:bg-gray-900">
    <div class="min-h-screen">
                ...
                <div class="flex items-center justify-between h-14 sm:h-16">
                    ...
                    <div class="flex items-center gap-2 sm:gap-4">
                        ...
                        $logoutUrl = $isOrchestratorOnly 
                            ? ADMIN_PATH . '/forms/orchestrator/login.php?form_id=' . urlencode($_SESSION['orchestrator_form_id'] ?? '')
                            : ADMIN_PATH . '/forms/logout.php';
                        ?>
                        <span
                        ...
                        <?php if ($isOrchestratorOnly): ?>
                            ...
                            function logoutOrchestrator() {
                                if (confirm('Are you sure you want to log out?')) {
                                    fetch('<?= ADMIN_PATH ?>/forms/orchestrator/api.php?action=orchestrator_logout', {
                                        method: 'POST',
        ...
        <!-- Broadcast Banner -->
        <div id="broadcastBanner" data-broadcast-id=""
            class="hidden fixed top-0 left-0 right-0 z-[100] bg-red-600 text-white text-sm font-medium shadow-lg transform transition-transform duration-300">
            <div class="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
                <span class="text-lg">📢</span>
                <span id="broadcastBannerMessage" class="flex-1"></span>
                <button onclick="dismissBroadcast()" class="p-1 hover:bg-red-500 rounded-lg transition-colors">
    ...
    function pollBroadcasts() {
        ...
        const formId = <?= json_encode($activeFormId ?? '') ?>;
        if (!formId) return;

        fetch('<?= ADMIN_PATH ?>/forms/orchestrator/api.php?action=get_recent_broadcasts&form_id=' + encodeURIComponent(formId))
            .then(r => r.json())
            .then(data => {
                if (!data.success) return;
                const dismissed = banner.dataset.broadcastId;
                const active = data.broadcasts?.find(b => b.id !== dismissed);
                if (active) {
                    banner.dataset.broadcastId = active.id;
                    document.getElementById('broadcastBannerMessage').textContent = active.message;
    ...
    broadcastTimer = setInterval(pollBroadcasts, 10000);
    pollBroadcasts();
    </script>
</body>

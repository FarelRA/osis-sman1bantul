...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($originalId) {
            foreach ($forms as $key => $form) {
                if ($form['id'] === $originalId) {
                    $forms[$key] = array_merge($form, $data);
                    $found = true;
                    break;
                }
            }
    ...
    if ($action === 'approve' || $action === 'reject') {
        ...
        if ($regId && $formId) {
            ...
            header('Location: ' . ADMIN_PATH . '/forms.php?view=registrations&form_id=' . urlencode($formId));
            exit;
        }
    }
    ...
    <?php else: ?>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                <a href="<?= ADMIN_PATH ?>/forms/settings.php"
                    ...
                    <div
                        ...
                        <?php if (!empty($form['registration_settings']['enabled'])): ?>
                            <div class="flex gap-2">
                                <a href="<?= ADMIN_PATH ?>/forms/registrations.php?form_id=<?= urlencode($form['id']) ?>"
                                ...
                                <a href="<?= ADMIN_PATH ?>/forms/orchestrator.php?form_id=<?= urlencode($form['id']) ?>"
                        ...
                        <?php else: ?>
                            <!-- Submissions Button for standalone forms -->
                            <a href="<?= ADMIN_PATH ?>/forms/submissions.php?form_id=<?= urlencode($form['id']) ?>"
                                class="flex-1 sm:flex-none justify-center px-4 py-2.5 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-900/40 rounded-lg flex items-center gap-2 transition-colors font-bold text-sm">
                                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div
                            ...
                            <div class="flex flex-wrap gap-1 mb-6 border-b border-gray-200 dark:border-gray-700">
                                <button @click="settingsTab = 'basic'"
                                    :class="settingsTab === 'basic' ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                                    class="px-3 py-2 text-sm font-bold transition-colors -mb-px">
                                    ⚙️ Basic
                                </button>
                                <button @click="settingsTab = 'registration'"
                                    :class="settingsTab === 'registration' ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                                    class="px-3 py-2 text-sm font-bold transition-colors -mb-px">
                                    🚀 Registration
                                </button>
                                <button @click="settingsTab = 'assignment'"
                                    :class="settingsTab === 'assignment' ? 'border-b-2 border-purple-500 text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                                    class="px-3 py-2 text-sm font-bold transition-colors -mb-px">
                                    🎫 Assignment
                                </button>
                                <button @click="settingsTab = 'stations'"
                                    :class="settingsTab === 'stations' ? 'border-b-2 border-purple-500 text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                                    class="px-3 py-2 text-sm font-bold transition-colors -mb-px">
                                    📡 Stations
                                </button>
                            </div>
                            ...
                            <!-- Tab: Basic -->
                            <div x-show="settingsTab === 'basic'" class="space-y-5">
                                <div>
                                    <label
                                ...
                                <p class="text-xs text-gray-400 text-center pt-2 border-t border-gray-100 dark:border-gray-800">
                            ...
                            <!-- Tab: Registration -->
                            <div x-show="settingsTab === 'registration'" class="space-y-5">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="font-bold text-lg text-gray-900 dark:text-white">🚀 Registration</h3>
                                ...
                                <div x-show="form.registration_settings?.enabled" x-transition class="space-y-5">
                                    ...
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        ...
                                        <div x-show="form.registration_settings.document_verification"
                                            ...
                                            <label class="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox"
                                                    x-model="form.registration_settings.ai_verification"
                                                ...
                                                <span class="text-xs text-gray-600 dark:text-gray-400">🤖 Enable AI
                                                    verification (requires API key in Settings)</span>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        <label class="flex items-center gap-2 cursor-pointer mb-2">
                                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">📷 OCR
                                                Mode</span>
                                        </label>
                                        <p class="text-xs text-gray-500 mb-3">How to extract text from ID card images in
                                            the scanner</p>
                                        <div class="space-y-2 pl-2">
                                            <label class="flex items-center gap-3 cursor-pointer p-2 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50 transition-colors">
                                                <input type="radio" name="ocr_mode" value="tesseract"
                                                    x-model="form.registration_settings.ocr_mode"
                                                    class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                                                <div>
                                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">📱
                                                        On-device (Tesseract.js)</span>
                                                    <p class="text-xs text-gray-500">Processes entirely in browser, no
                                                        server call</p>
                                                </div>
                                            </label>
                                            <label class="flex items-center gap-3 cursor-pointer p-2 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50 transition-colors">
                                                <input type="radio" name="ocr_mode" value="ai"
                                                    x-model="form.registration_settings.ocr_mode"
                                                    class="w-4 h-4 text-purple-600 focus:ring-purple-500 border-gray-300">
                                                <div>
                                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">🤖
                                                        AI OCR (Server-side LLM)</span>
                                                    <p class="text-xs text-gray-500">Sends image to LLM for extraction
                                                        (requires API key in Settings)</p>
                                                </div>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox"
                                                x-model="form.registration_settings.merch_enabled"
                                                x-init="if(form.registration_settings.merch_enabled === undefined) form.registration_settings.merch_enabled = true"
                                                class="w-5 h-5 rounded text-yellow-500 focus:ring-yellow-500 border-gray-300">
                                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">🎁 Merch
                                                Distribution</span>
                                        </label>
                                        <p class="text-xs text-gray-500 pl-7">Enable merch/giveaway distribution feature
                                            in the scanner</p>
                                    </div>
                                    ...
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox"
                                                x-model="form.registration_settings.payment_enabled"
                                                class="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-gray-300">
                                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">💳 Payment
                                                Step</span>
                                        </label>
                                        <p class="text-xs text-gray-500 pl-7">Adds a payment selection & verification
                                            step</p>
                                        <div x-show="form.registration_settings.payment_enabled" class="pl-7 space-y-3">
                                            ...
                                            <div>
                                                <label class="block text-xs font-bold text-gray-600 mb-1">Offline
                                                    Location</label>
                                                <input type="text"
                                    ...
                                    <!-- Completion Settings -->
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        <h4 class="text-sm font-bold text-gray-700 dark:text-gray-300">✅ Completion
                                            Settings</h4>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1">Completion
                                                Message</label>
                                            <input type="text"
                                                x-model="form.registration_settings.completion_message"
                                        ...
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1">WhatsApp Group
                                                Link</label>
                                            <input type="url"
                                                x-model="form.registration_settings.whatsapp_group_link"
                                    ...
                                    <!-- Timeout Settings -->
                                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl space-y-3">
                                        <h4 class="text-sm font-bold text-gray-700 dark:text-gray-300">⏱️ Timeout
                                            Settings</h4>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1">Default Step
                                                Timeout (seconds)</label>
                                            <input type="number"
                                                x-model="form.registration_settings.default_timeout"
                                                placeholder="900 (15 minutes)"
                                                class="w-full px-3 py-2 border-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-100 dark:border-gray-700 focus:border-blue-500 text-sm">
                                            <p class="text-xs text-gray-500 mt-1">0 = No timeout. Default: 900 (15 min)
                                            </p>
                                        </div>
                                        <div x-show="form.registration_settings.document_verification">
                                            <label class="block text-xs font-bold text-gray-600 mb-1">Document Upload
                                                Timeout (seconds)</label>
                                            <input type="number"
                                                x-model="form.registration_settings.document_timeout"
                                                placeholder="3600 (1 hour)"
                                                class="w-full px-3 py-2 border-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-100 dark:border-gray-700 focus:border-blue-500 text-sm">
                                            <p class="text-xs text-gray-500 mt-1">0 = No timeout. Default: 3600 (1 hour)
                                            </p>
                                        </div>
                                        <div x-show="form.registration_settings.payment_enabled">
                                            <label class="block text-xs font-bold text-gray-600 mb-1">Payment Step
                                                Timeout (seconds)</label>
                                            <input type="number"
                                                x-model="form.registration_settings.payment_timeout"
                                                placeholder="7200 (2 hours)"
                                                class="w-full px-3 py-2 border-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-100 dark:border-gray-700 focus:border-blue-500 text-sm">
                                            <p class="text-xs text-gray-500 mt-1">0 = No timeout. Default: 7200 (2
                                                hours)</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Tab: Assignment -->
                            <div x-show="settingsTab === 'assignment'" class="space-y-5">
                                ...
                                <div x-show="form.registration_settings?.assignment_settings?.enabled"
                                    ...
                                    <!-- Classes -->
                                    <div>
                                        <label class="block text-xs font-bold text-gray-600 mb-2">Classes</label>
                            ...
                            <!-- Tab: Stations -->
                            <div x-show="settingsTab === 'stations'" class="space-y-5">
                                <div class="flex items-center gap-3 mb-4">
                                    <span class="font-bold text-lg text-gray-900 dark:text-white">📡 Scanner
                                        Stations</span>
                                </div>
                                <p class="text-xs text-gray-500">
                                    Define stations available for orchestrator operators. Each station needs an emoji
                                    and a label.
                                </p>
                                <div class="space-y-2 max-h-60 overflow-y-auto">
                                    <template x-for="(station, idx) in (form.stations || [])" :key="idx">
                                        <div class="flex gap-2 items-center">
                                            <input type="text" x-model="station.emoji" placeholder="🚪"
                                                class="w-12 px-2 py-1.5 border-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-100 dark:border-gray-700 focus:border-purple-500 text-sm text-center">
                                            <input type="text" x-model="station.label"
                                                placeholder="Gerbang Masuk"
                                                class="flex-1 px-2 py-1.5 border-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-100 dark:border-gray-700 focus:border-purple-500 text-sm">
                                            <button type="button"
                                                @click="form.stations.splice(idx, 1)"
                                                class="p-1.5 text-red-500 hover:bg-red-100 rounded-lg">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <button type="button"
                                    @click="if(!form.stations) form.stations = []; form.stations.push({emoji:'', label:''})"
                                    class="w-full py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-xs font-bold text-gray-500 hover:text-purple-500 hover:border-purple-500 transition-all">
                                    + Add Station
                                </button>
                            </div>
                        </div>

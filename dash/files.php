...
$dir = Security::sanitizeFilename($_GET['dir'] ?? '');
$baseAllowed = realpath(BASE_PATH);
...
if ($dir === '' || $dir === '~') {
    ...
    if (str_starts_with($dir, '~')) {
        ...
        $candidate = realpath(BASE_PATH . '/' . $rel);
    } else {
        $candidate = realpath(BASE_PATH . '/' . $dir);
    }
    $path = ($candidate !== false && str_starts_with($candidate, $baseAllowed) && is_dir($candidate))
        ? $candidate
        : $baseAllowed;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action'])) {
    if (!CSRF::verify($_POST['csrf_token'] ?? '')) {
        json_response(['message' => 'Invalid form token'], 403);
    }
    ...
    if ($action === 'direct_upload') {
        $filename = Security::sanitizeFilename(basename($_GET['filename'] ?? ''));
        if ($filename === '') {
    ...
    if ($action === 'rename') {
        $oldName = Security::sanitizeFilename(trim(basename($_POST['old_name'] ?? '')));
        $newName = trim(basename($_POST['new_name'] ?? ''));
        if ($oldName === '' || $newName === '') {
            json_response(['message' => 'Missing rename values'], 400);
        }

        $oldFile = realpath($path . '/' . $oldName);
        $newFile = $path . '/' . Security::sanitizeFilename(basename($newName));
        $baseAllowed = realpath(BASE_PATH);
        if ($oldFile === false || !str_starts_with($oldFile, $baseAllowed) || !str_starts_with(realpath(dirname($newFile)), $baseAllowed)) {
    ...
    if ($action === 'save_file') {
        $filename = Security::sanitizeFilename(trim(basename($_POST['filename'] ?? '')));
        if ($filename === '') {
            json_response(['message' => 'Missing filename'], 400);
        }
...
<div x-data="fileManager(<?= htmlspecialchars(json_encode([
    ...
    <div class="flex justify-between items-center mb-6 sm:mb-8">
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">File Manager</h2>
        <a href="<?= ADMIN_PATH ?>/" class="btn-secondary text-sm">Back to Admin</a>
    </div>
...
function fileManager(initialState) {
    return {
        ...
        csrfToken: initialState.csrf_token,
        editor: null,
        message: '',
